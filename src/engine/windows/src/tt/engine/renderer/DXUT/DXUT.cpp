...
bool g_fullscreenBorderlessWindow = false;
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTCreateWindow( const WCHAR* strWindowTitle, HINSTANCE hInstance, HICON hIcon,
	                             HMENU hMenu, int x, int y, bool p_resizable, bool p_fullscreenBorderless )
{
    g_fullscreenBorderlessWindow = p_fullscreenBorderless;
    HRESULT hr;
    ...
    if( DXUTGetHWNDFocus() == NULL )
    {
        ...
        wndClass.hCursor = NULL; // NOTE: Must use NULL here to allow custom HW cursor
		...
		if (g_fullscreenBorderlessWindow)
		{
        wndStyle &= ~(WS_CAPTION | WS_THICKFRAME | WS_MINIMIZE | WS_MAXIMIZE | WS_SYSMENU);
                          ...
                          bool bForceRecreate, bool bClipWindowToSingleAdapter )
{
        ...
        DXUTCanDeviceBeReset( pOldDeviceSettings, pNewDeviceSettings, pd3d9DeviceFromApp, pd3d10DeviceFromApp ) )
    {
        ...
        if( FAILED( hr ) )
        {
            ...
            else // DXUTERR_RESETTINGDEVICE
            {
                ...
                {
                    SAFE_DELETE(pNewDeviceSettingsOnHeap);
        ...
        !bKeepCurrentWindowSize )                      // only resize if pp.BackbufferWidth/Height were not 0
    {
        ...
        if( bClipWindowToSingleAdapter && !IsIconic( DXUTGetHWNDDeviceWindowed() ) )
        {
            ...
            HMONITOR attachedMonitor = DXUTGetMonitorFromAdapter( pNewDeviceSettings );
            DXUTGetMonitorInfo(attachedMonitor, &miAdapter );
            ...
            {
                if( hWindowMonitor == attachedMonitor && IsZoomed( DXUTGetHWNDDeviceWindowed() ) )
                {
    ...
    if( bNeedToResize )
    {
        ...
        if( bClipWindowToSingleAdapter )
        {
            ...
            // after resetting the device rather than beforehand to ensure 
            // that the monitor resolution is correct and does not limit the size of the new window.
			if (g_fullscreenBorderlessWindow == false)
			{
...
void DXUTCleanup3DEnvironment9( bool bReleaseSettings )
{
    ...
    if( pd3dDevice != NULL )
    {
        ...
        if( pd3dDevice )
        {
            ...
            if( references > 0 )
            {
                TT_PANIC("Found non-zero reference count in DXUTCleanup3DEnvironment9!");
#if !defined(TT_BUILD_FINAL) // Would trigger in final builds for Toki and RUSH.
...
void DXUTAllowShortcutKeys( bool bAllowKeys )
{
    ...
    else
    {
        ...
        if( GetDXUTState().GetKeyboardHook() == NULL )
        {
            ...
            // MARTIJN VS2013: Disabled the check (deprecated)
...
void DXUTResizeDXGIBuffers( UINT Width, UINT Height, BOOL bFullScreen )
{
    ...
    if( FAILED( hr ) )
    {
        ...
        GetDXUTState().SetInsideDeviceCallback( true );
        LPDXUTCALLBACKD3D10SWAPCHAINRELEASING callback =
            GetDXUTState().GetD3D10SwapChainReleasingFunc();
        if(callback != NULL )
			callback( GetDXUTState().GetD3D10SwapChainResizedFuncUserContext() );
        GetDXUTState().SetInsideDeviceCallback( false );
        DXUTPause( false, false );
        PostQuitMessage( 0 );
    }
...
void WINAPI DXUTChangeWindowSize(int p_newWidth, int p_newHeight)
{
	// Only possible in windowed mode!
	if(DXUTIsWindowed() == false)
	{
		return;
	}

	// Must have positive values, greater than 0
    ...
    if(DXUTIsD3D9(&deviceSettings))
	{
        deviceSettings.d3d9.pp.BackBufferWidth  = p_newWidth;
		deviceSettings.d3d9.pp.BackBufferHeight = p_newHeight;
	}
    else
	{
		deviceSettings.d3d10.sd.BufferDesc.Width = p_newWidth;
		deviceSettings.d3d10.sd.BufferDesc.Height = p_newHeight;
	}
	...
	// Find valid device settings based on input
    hr = DXUTFindValidDeviceSettings(&deviceSettings, &deviceSettings, &matchOptions);
    ...
    if(SUCCEEDED(hr))
    {
        // Create a Direct3D device using the new device settings.  
        // If there is an existing device, then it will either reset or recreate the scene.
        hr = DXUTChangeDevice(&deviceSettings, 0, 0, false, false);

        // If hr == E_ABORT, this means the app rejected the device settings in the ModifySettingsCallback so nothing changed
        if(FAILED(hr) && (hr != E_ABORT))
        {
            // Failed creating device, try to switch back.
            HRESULT hr2 = DXUTChangeDevice(&orginalDeviceSettings, 0, 0, false, false);
...
void WINAPI DXUTForceDeviceReset()
{
	DXUTDeviceSettings deviceSettings = DXUTGetDeviceSettings();
	DXUTChangeDevice( &deviceSettings, NULL, NULL, true, false );
}
