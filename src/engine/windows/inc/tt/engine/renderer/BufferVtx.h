...
public:
	...
	pos(p_pos)
	{
		setColor(p_col); // Calling function instead of using initializer list to reduce ifdefs
		tex[0] = p_tex1;
	}
	...
	// Color functions (grouped together to limit the number of ifdefs)
	// Windows uses BGRA
	inline void setColor(const ColorRGBA& p_color)
	{ col.setColor(p_color.b, p_color.g, p_color.r, p_color.a); }
	
	inline void setColor(u8 p_r, u8 p_g, u8 p_b, u8 p_a) { col.setColor(p_b, p_g, p_r, p_a); }
	
	inline ColorRGBA getColor() const { return ColorRGBA(col.b, col.g, col.r, col.a); }
