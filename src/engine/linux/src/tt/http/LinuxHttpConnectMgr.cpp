...
static void disableSteamRuntime()
{
	// Steam runtime jailbreak fiasco
	char* ldLibPath = getenv("SYSTEM_LD_LIBRARY_PATH");
	if (ldLibPath) setenv("LD_LIBRARY_PATH", ldLibPath, 1);
	char* path = getenv("SYSTEM_PATH");
...
void HttpConnectMgr::openUrlExternally(const std::string& p_url)
{
	...
	if (pID == 0)
	{
		...
		execlp("/bin/bash", "xdg-open", "xdg-open", p_url.c_str(), (char *)0);
