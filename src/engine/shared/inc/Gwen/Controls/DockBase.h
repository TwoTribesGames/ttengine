	...
	namespace Controls
	{
		...
		class GWEN_EXPORT DockBase : public Base
		{
			public:
				...
				// Two Tribes addition: make it possible to query whether docks exist without creating them
				virtual bool HasRight()  { return HasChildDock( Pos::Right );  }
				virtual bool HasLeft()   { return HasChildDock( Pos::Left );   }
				virtual bool HasTop()    { return HasChildDock( Pos::Top );    }
				virtual bool HasBottom() { return HasChildDock( Pos::Bottom ); }
				// END Two Tribes addition
