#if !defined(INC_TT_AUDIO_CODEC_TTADPCM_TTADPCMDECODER_H)
#define INC_TT_AUDIO_CODEC_TTADPCM_TTADPCMDECODER_H
...
class TTAdpcmDecoder : public Decoder
{
public:
	explicit TTAdpcmDecoder(const std::string& p_filename, fs::identifier p_fs = 0);
	virtual ~TTAdpcmDecoder();
...
private:
	TTAdpcmDecoder(const TTAdpcmDecoder&);
	const TTAdpcmDecoder& operator=(const TTAdpcmDecoder&);
	...
	typedef std::vector<ADPCMState> DecodeStates;
...
#endif // !defined(INC_TT_AUDIO_CODEC_TTADPCM_TTADPCMDECODER_H)
