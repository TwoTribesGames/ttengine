#ifndef INC_TT_AUDIO_XACT_VOLUMEEVENT_H
#define INC_TT_AUDIO_XACT_VOLUMEEVENT_H
...
public:
	...
	static VolumeEvent* createVolumeEvent(const xml::XmlNode* p_node);
	
	VolumeEventInstance* instantiate(TrackInstance* p_track);
	
private:
	VolumeEvent(const VolumeEvent&);
	VolumeEvent& operator=(const VolumeEvent&);
	...
	// equation variables
	real m_value;    // -96.0 - 6.0 (only when equationtype is value)
	real m_rangeMin; // -96.0 - 6.0 (only when equationtype is random)
	real m_rangeMax; // -96.0 - 6.0 (only when equationtype is random)
	
	// ramp variables
	real m_init;       // -96.0 - 6.0  The starting pitch of this volume ramp.
...
#endif // INC_TT_AUDIO_XACT_VOLUMEEVENT_H
