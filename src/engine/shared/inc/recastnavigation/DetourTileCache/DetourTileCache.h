...
{
	float pos[3], radius, height;  // for cylindrical obstacle. In case of box, "pos" is the minimum position of the box
	bool  isBox;     // whether obstacle is specified as a box (minPos, maxPos) or cylinder (pos, radius, height)
	float maxPos[3]; // for box obstacle
...
public:
	...
	inline int getPendingUpdateCount() const { return m_nupdate; } // Added by Two Tribes.
	...
	dtStatus addObstacle(const float* minPos, const float* maxPos, dtObstacleRef* result);
...
private:
	...
	static const int MAX_REQUESTS = 512; //64;   // Two Tribes change: increase capacity
	ObstacleRequest m_reqs[MAX_REQUESTS];
	int m_nreqs;
	
	static const int MAX_UPDATE = 512; //64;   // Two Tribes change: increase capacity
