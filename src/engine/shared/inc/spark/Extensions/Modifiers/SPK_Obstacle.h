...
namespace SPK
{
	...
	*/
	...
	inline Obstacle* Obstacle::create(Zone* p_zone,ModifierTrigger p_trigger,float p_bouncingRatio,float p_friction)
	{
		Obstacle* obj = new Obstacle(p_zone,p_trigger,p_bouncingRatio,p_friction);
		registerObject(obj);
		return obj;
	}
		
	inline void Obstacle::setBouncingRatio(float p_bouncingRatio)
	{
		bouncingRatio = p_bouncingRatio < 0.0f ? 0.0f : p_bouncingRatio;
	}

	inline void Obstacle::setFriction(float p_friction)
	{
		friction = p_friction;
	}
