...
namespace SPK
{
	...
	*/
	...
	inline Collision* Collision::create(float p_scale,float p_elasticity)
	{
		Collision* obj = new Collision(p_scale,p_elasticity);
		registerObject(obj);
		return obj;
	}

	inline void Collision::setElasticity(float p_elasticity)
	{
		if (p_elasticity >= 0.0f)
			elasticity = p_elasticity;
	}

	inline void Collision::setScale(float p_scale)
	{
		scale = p_scale;
	}
