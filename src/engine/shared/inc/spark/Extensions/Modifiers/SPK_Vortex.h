...
namespace SPK
{
	...
	*/
	...
	inline Vortex* Vortex::create(const Vector3D& p_position,const Vector3D& p_direction,float p_rotationSpeed,float p_attractionSpeed)
	{
		Vortex* obj = new Vortex(p_position,p_direction,p_rotationSpeed,p_attractionSpeed);
		registerObject(obj);
		return obj;
	}

	inline void Vortex::setPosition(const Vector3D& p_position)
	{
		position  = p_position;
		tPosition = position;
		notifyForUpdate();
	...
	inline void Vortex::setDirection(const Vector3D& p_direction)
	{
	...
	inline void Vortex::setRotationSpeed(float p_rotationSpeed, bool p_angular)
	{
		rotationSpeed       = p_rotationSpeed;
		angularSpeedEnabled = p_angular;
	}

	inline void Vortex::setAttractionSpeed(float p_attractionSpeed, bool p_linear)
	{
		attractionSpeed    = p_attractionSpeed;
		linearSpeedEnabled = p_linear;
	}

	inline void Vortex::setEyeRadius(float p_eyeRadius)
	{
		if (p_eyeRadius < 0.0f) p_eyeRadius = -p_eyeRadius;
		eyeRadius = p_eyeRadius;
	}
