...
namespace SPK
{
	...
	*/
	...
	inline LinearForce* LinearForce::create(Zone* p_zone,ModifierTrigger p_trigger,const Vector3D& p_force,ForceFactor p_type,ModelParam p_param)
	{
		LinearForce* obj = new LinearForce(p_zone,p_trigger,p_force,p_type,p_param);
		registerObject(obj);
		return obj;
	}
	
	inline void LinearForce::setForce(const Vector3D& p_force)
	{
		force = tForce = p_force;
		notifyForUpdate();
	}
