...
namespace SPK
{
	...
	*/
	...
	inline PointRendererInterface::PointRendererInterface(PointType p_type, float p_size) :
		type(p_type),
		size(p_size)
	{}

	inline bool PointRendererInterface::setType(PointType p_type)
	{
		type = p_type;
		return true;
	}

	inline void PointRendererInterface::setSize(float p_size)
	{
		size = p_size;
	}
