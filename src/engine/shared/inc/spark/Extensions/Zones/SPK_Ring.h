...
namespace SPK
{
	...
	*/
	...
	inline Ring* Ring::create(const Vector3D& p_position,const Vector3D& p_normal,float p_minRadius,float p_maxRadius)
	{
		Ring* obj = new Ring(p_position,p_normal,p_minRadius,p_maxRadius);
		registerObject(obj);
		return obj;
	}
		
	inline void Ring::setNormal(const Vector3D& p_normal)
	{
		normal = p_normal;
		normal.normalize();
		tNormal = normal;
