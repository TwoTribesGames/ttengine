...
namespace SPK
{
	...
	*/
	...
	inline Cylinder* Cylinder::create(const Vector3D& p_position,const Vector3D& p_direction,float p_radius,float p_length)
	{
		Cylinder* obj = new Cylinder(p_position,p_direction,p_radius,p_length);
		registerObject(obj);
		return obj;
	}

	inline void Cylinder::setDirection(const Vector3D& p_direction)
	{
	...
	inline void Cylinder::setRadius(float p_radius)
	{
	    radius = (p_radius < 0 ? -p_radius : p_radius);
	}

	inline void Cylinder::setLength(float p_length)
	{
	    length = (p_length < 0 ? -p_length : p_length);
	}
