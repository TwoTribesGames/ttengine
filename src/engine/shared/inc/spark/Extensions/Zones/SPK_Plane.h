...
namespace SPK
{
	...
	*/
	...
	inline Plane* Plane::create(const Vector3D& p_position,const Vector3D& p_normal)
	{
		Plane* obj = new Plane(p_position,p_normal);
		registerObject(obj);
		return obj;
	}
		
	inline void Plane::setNormal(const Vector3D& p_normal)
	{
		normal = p_normal;
		normal.normalize();
		tNormal = normal;
