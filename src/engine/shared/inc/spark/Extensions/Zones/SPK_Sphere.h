...
namespace SPK
{
	...
	*/
	...
	inline Sphere* Sphere::create(const Vector3D& p_position,float p_radius)
	{
		Sphere* obj = new Sphere(p_position,p_radius);
		registerObject(obj);
		return obj;
	}
		
	inline void Sphere::setRadius(float p_radius)
	{
		radius = p_radius >= 0.0f ? p_radius : 0.0f;
	}
