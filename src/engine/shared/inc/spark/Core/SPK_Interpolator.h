...
namespace SPK
{
	...
	*/
	struct InterpolatorEntry
	{
		...
		*/
		InterpolatorEntry(float p_x, float p_y) : x(p_x),y0(p_y),y1(p_y) {}
		...
		*/
		InterpolatorEntry(float p_x, float p_y0, float p_y1) : x(p_x),y0(p_y0),y1(p_y1) {}

		// used internally
		InterpolatorEntry(float p_x) : x(p_x) {}
	};
	...
	*/
	...
	inline void Interpolator::setType(InterpolationType p_type, ModelParam p_param)
	{
		type  = p_type;
		param = p_param;
	}
	...
	inline void Interpolator::setScaleXVariation(float p_scaleXVariation)
	{
		scaleXVariation = p_scaleXVariation;
	}

	inline void Interpolator::setOffsetXVariation(float p_offsetXVariation)
	{
		offsetXVariation = p_offsetXVariation;
	}
