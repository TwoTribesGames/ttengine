...
namespace SPK
{
	...
	*/
	...
	inline Model* Model::create(int p_enableFlag,int p_mutableFlag,int p_randomFlag,int p_interpolatedFlag)
	{
		Model* obj = new Model(p_enableFlag,p_mutableFlag,p_randomFlag,p_interpolatedFlag);
		registerObject(obj);
		return obj;
	}
	
	inline void Model::setLifeTime(float p_lifeTimeMin, float p_lifeTimeMax)
	{
		lifeTimeMin = p_lifeTimeMin;
		lifeTimeMax = p_lifeTimeMax;
	}

	inline void Model::setImmortal(bool p_immortal)
	{
		immortal = p_immortal;
	}
