...
namespace SPK
{
	...
	*/
	...
	template<class T>
	ArrayBuffer<T>::ArrayBuffer(size_t p_nbParticles, size_t p_particleSize) :
		Buffer(),
		particleSize(p_particleSize),
		dataSize(p_nbParticles * p_particleSize)
	{
		data = new T[dataSize];
	}
	...
	template<class T>
	ArrayBufferCreator<T>::ArrayBufferCreator(size_t p_particleSize) :
		BufferCreator(),
		particleSize(p_particleSize)
	{}
