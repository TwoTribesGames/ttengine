...
#include <spark/Core/SPK_Registerable.h>
#include <spark/Core/SPK_Transformable.h>
...
namespace SPK
{
	...
	*/
	...
	protected :
		...
		void copyChildren(const Group& group,bool createBase);
	...
	inline Group* Group::create(Model* p_model,size_t p_capacity)
	{
		Group* obj = new Group(p_model,p_capacity);
		registerObject(obj);
		return obj;
	}

	inline void Group::setFriction(float p_friction)
	{
		friction = p_friction;
	}

	inline void Group::setGravity(const Vector3D& p_gravity)
	{
		gravity = p_gravity;
	}

	inline void Group::setCustomUpdate(bool (*p_fupdate)(Particle&,float))
	{
		fupdate = p_fupdate;
	}

	inline void Group::setCustomBirth(void (*p_fbirth)(Particle&))
	{
		fbirth = p_fbirth;
	}

	inline void Group::setCustomDeath(void (*p_fdeath)(Particle&))
	{
		fdeath = p_fdeath;
	}

	inline void Group::enableSorting(bool p_sort)
	{
		sortingEnabled = p_sort;
		distanceComputationEnabled = p_sort;
	}
