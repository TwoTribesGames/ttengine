...
namespace SPK
{
	...
	*/
	...
	protected :
		...
		* @param createBase : true if a base is created, false otherwise
		*/
		void copyChildren(const Registerable& /*object*/,bool /*createBase*/) { }
	...
	inline void Registerable::setShared(bool p_shared)
	{
		shared = p_shared;
	}
	
	inline void Registerable::setDestroyable(bool p_destroyable)
	{
		destroyable = p_destroyable;
	}

	inline void Registerable::setName(const std::string& p_name)
	{
		name = p_name;
	}
	...
	inline Registerable* Registerable::findByName(const std::string& p_name)
	{
		return getName().compare(p_name) == 0 ? this : NULL;
	}
