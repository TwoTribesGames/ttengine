...
SQTable *SQTable::Clone()
{
    ...
    for(n = 0; n < _numofnodes; n++) {
        ...
        dst->val = src->val;
        if(src->next) {
            SQ_ASSERT(src->next > basesrc);
            dst->next = basedst + (src->next - basesrc);
            SQ_ASSERT(dst != dst->next);
        }
        dst++;
        src++;
    }
    SQ_ASSERT(_firstfree > basesrc);
    SQ_ASSERT(_firstfree != NULL);
