...
SQInteger SQClass::Next(bool getweakrefs, const SQObjectPtr &refpos, SQObjectPtr &outkey, SQObjectPtr &outval)
{
	SQObjectPtr oval;
	...
	if(idx != -1) {
		if(_ismethod(oval)) {
			...
			outval = getweakrefs ? o : _realval(o);
		}
    }
    return idx;
}
...
// Two Tribes change
bool SQClass::ContainsBase(SQClass *base)
{
	SQClass *parent = this;
	while(parent != NULL) {
		if(parent == base)
