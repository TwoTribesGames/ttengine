...
static bool sq_aux_gettypedarg(HSQUIRRELVM v,SQInteger idx,SQObjectType type,SQObjectPtr **o)
{
    *o = &stack_get(v,idx);
    if(type(**o) != type){
		...
		//TT_PANIC("wrong argument type, expected '%s' got '%.50s'", IdType2Name(type),_stringval(oval));
...
// Two Tribes change
SQBool sq_containsbase(HSQUIRRELVM v)
{
	SQObjectPtr &cl = stack_get(v,-1);
	SQObjectPtr &clBase = stack_get(v,-2);
	if(type(cl) != OT_CLASS || type(clBase) != OT_CLASS)
		return sq_throwerror(v,_SC("invalid param type"));
	return _class(cl)->ContainsBase(_class(clBase))?SQTrue:SQFalse;
}
...
// Add by Two Tribes
HSQOBJECT sq_getrealval(const HSQOBJECT *o)
{
	return _realval(*o);
}
...
SQRESULT sq_get_getweakrefs(HSQUIRRELVM v,SQInteger idx)
{
	SQObjectPtr &self=stack_get(v,idx);
	if(v->Get(self,v->GetUp(-1),v->GetUp(-1),0,DONT_FALL_BACK, true))
		return SQ_OK;
	v->Pop();
	return SQ_ERROR;
}
...
bool sq_isfield(HSQUIRRELVM v,SQInteger idx) // ADDED BY TWO TRIBES
{
	SQObjectPtr &self=stack_get(v,idx);
	switch (type(self))
	{
	case OT_INSTANCE:
		{
			const SQObjectPtr& key = v->Top();
			      SQObjectPtr val;
			return _instance(self)->_class->_members->Get(key,val) && _isfield(val) != 0;
		}
	case OT_CLASS:
		{
			const SQObjectPtr& key = v->Top();
			      SQObjectPtr val;
			return _class(self)->_members->Get(key,val) && _isfield(val) != 0;
		}
	default:
		sq_throwerror(v,_SC("invalid param type"));
		return false;
	}
}
...
SQRESULT sq_rawget(HSQUIRRELVM v,SQInteger idx)
{
    ...
    case OT_CLASS:
        ...
        break;
    case OT_INSTANCE:
        if(_instance(self)->Get(obj,obj,false))
            return SQ_OK;
        break;
    case OT_ARRAY:{
        if(sq_isnumeric(obj)){
...
SQRESULT sq_throwerror(HSQUIRRELVM v,const SQChar *err)
{
	...
	//TT_PANIC("sq_throwerror: %s", v->_lasterror._unVal.pString->_val);
    return SQ_ERROR;
}
...
SQRESULT sq_next_impl(HSQUIRRELVM v,SQInteger idx, bool getweakrefs)
{
	...
	int faketojump;
	if(!v->FOREACH_OP(o,realkey,val,refpos,0,666,faketojump, getweakrefs))
		return SQ_ERROR;
	if(faketojump != 666) {
		v->Push(realkey);
		v->Push(val);
...
{
	return sq_next_impl(v, idx, false);
}

SQRESULT sq_next_getweakrefs(HSQUIRRELVM v,SQInteger idx)
{
	return sq_next_impl(v, idx, true);
}
