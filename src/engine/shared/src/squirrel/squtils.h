...
public:
    ...
    inline T &back() const { SQ_ASSERT(_size > 0);  return _vals[_size - 1]; }
    inline T& operator[](SQUnsignedInteger pos) const{ SQ_ASSERT(pos >= 0 && pos < _size); return _vals[pos]; }
    T* _vals;
private:
