...
bool SQVM::ArithMetaMethod(SQInteger op,const SQObjectPtr &o1,const SQObjectPtr &o2,SQObjectPtr &dest)
{
    ...
    switch(op){
        ...
        default: mm = MT_ADD; SQ_ASSERT(0); break; //shutup compiler
    }
...
bool SQVM::StartCall(SQClosure *closure,SQInteger target,SQInteger args,SQInteger stackbase,bool tailcall)
{
    ...
    if(func->_varparams)
    {
        paramssize--;
        if (nargs < paramssize) {
			Raise_Error(_SC("Function '%s()': wrong number of parameters. Expected %d, got %d\nFile: '%s' near line: %d\n"),
				_stringval_safe(func->_name), paramssize-1, nargs-1, _stringval_safe(func->_sourcename),
				func->GetLine(func->_instructions));
            return false;
        }
...
&o3,SQObjectPtr &o4,SQInteger SQ_UNUSED_ARG(arg_2),int exitpos,int &jump, bool getweakrefs)
{
    SQInteger nrefidx;
    switch(type(o1)) {
    case OT_TABLE:
        if((nrefidx = _table(o1)->Next(getweakrefs, o4, o2, o3)) == -1) _FINISH(exitpos);
        o4 = (SQInteger)nrefidx; _FINISH(1);
    case OT_ARRAY:
        if((nrefidx = _array(o1)->Next(getweakrefs, o4, o2, o3)) == -1) _FINISH(exitpos);
        o4 = (SQInteger) nrefidx; _FINISH(1);
    case OT_STRING:
        if((nrefidx = _string(o1)->Next(o4, o2, o3)) == -1)_FINISH(exitpos);
        o4 = (SQInteger)nrefidx; _FINISH(1);
    case OT_CLASS:
        if((nrefidx = _class(o1)->Next(getweakrefs, o4, o2, o3)) == -1)_FINISH(exitpos);
        o4 = (SQInteger)nrefidx; _FINISH(1);
    case OT_USERDATA:
    case OT_INSTANCE:
        if(_delegable(o1)->_delegate) {
        ...
        for(;;)
        {
            ...
            case _OP_APPENDARRAY:
                ...
                case AAT_BOOL:
                    ...
                    break;
                default: val._type = OT_INTEGER; SQ_ASSERT(0); break;
            ...
            case _OP_FOREACH:{ int tojump;
                _GUARD(FOREACH_OP(STK(arg0),STK(arg2),STK(arg2+1),STK(arg2+2),arg2,sarg1,tojump,false));
                ci->_ip += tojump; }
                continue;
            case _OP_POSTFOREACH:
                SQ_ASSERT(type(STK(arg0)) == OT_GENERATOR);
...
bool SQVM::Get(const SQObjectPtr &self, const SQObjectPtr &key, SQObjectPtr &dest, SQUnsignedInteger getflags, SQInteger selfidx, bool getweakrefs)
{
	SQ_ASSERT(getweakrefs == false || type(self) == OT_INSTANCE); // Added by Two Tribes. We don't implement getweakrefs for non instance type.
    ...
    case OT_ARRAY:
        ...
        break;
    case OT_INSTANCE:
        if(_instance(self)->Get(key,dest,getweakrefs)) return true;
        break;
    case OT_CLASS:
...
#ifdef _DEBUG
    if(!_suspended) {
        SQ_ASSERT(_stackbase == prevstackbase);
    }
#endif
    return true;
}
