...
function build_tree()
{
	foreach(i,o in objs_reg.refs)
	{
		...
		if(i != ::getroottable())
		{
			// Check if a _serialize method exists, if so use that one first
			if ("_serialize" in i)
			{
				foreach(element in i._serialize())
				{
					beginelement("e");
					foreach (attr in element)
					{
						attribute(attr[0].tostring(), attr[1].tostring());
					}
					endelement("e");
				}		
			}
			...
			iterateobject(i,function (obj,idx,val) 
			{
				...
				// Skip sqbind tables
				if (idx == "__member_variables_set" || idx == "__member_variables_offset" || idx == "__member_variables_get")
					return;
