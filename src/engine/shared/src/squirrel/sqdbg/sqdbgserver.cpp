...
#include <squirrel/sqdbg/sqdbgserver.h>
...
{
	std::string srcLCase(tt::str::toLower(src));
	
	VMState *vs = GetVMState(v);
	switch(_state){
	case eDBG_Running:
		if(type==_SC('l') && _breakpoints.size()) {
			BreakPointSetItor itr = _breakpoints.find(BreakPoint(line,srcLCase.c_str()));
			if(itr != _breakpoints.end()) {
				Break(v,line,srcLCase.c_str(),_SC("breakpoint"));
				BreakExecution();
			}
		}
		break;
	...
	case eDBG_StepOver:
		...
		case _SC('l'):
			if(vs->_nestedcalls==0) {
				Break(v,line,srcLCase.c_str(),_SC("step"));
				BreakExecution();
			}
			break;
		case _SC('c'):
...
bool SQDbgServer::ParseBreakpoint(const char *msg,BreakPoint &out)
{
	...
	while((*ep)!='\n' && (*ep)!='\0')
	{
		*dest=(char)tolower(*ep);
		*dest++;*ep++;
	}
	*dest='\0';
	*dest++;
	*dest='\0';
	out._src = tt::str::toLower(std::string(stemp));
	return true;
}
...
void SQDbgServer::BeginElement(const SQChar *name)
{
	...
	sprintf(&_scratchstring[0],_SC("<%s"),name);
...
{
	XMLElementState *self = &xmlstate[_xmlcurrentement];
	SQ_ASSERT(!self->haschildren); //cannot have attributes if already has children
	const SQChar *escval = escape_xml(value);
	_scratchstring.resize(10+scstrlen(name)+scstrlen(escval));
	sprintf(&_scratchstring[0],_SC(" %s=\"%s\""),name,escval);
	SendChunk(&_scratchstring[0]);
}
...
{
	XMLElementState *self = &xmlstate[_xmlcurrentement];
	SQ_ASSERT(scstrcmp(self->name,name) == 0);
	if(self->haschildren) {
