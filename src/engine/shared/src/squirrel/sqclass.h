...
public:
	...
	// Two Tribes change
	bool ContainsBase(SQClass *base);
...
public:
    ...
    ~SQInstance();
	bool Get(const SQObjectPtr &key,SQObjectPtr &val, bool getweakrefs)  { // getweakrefs Added by Two Tribes
		if(_class->_members->Get(key,val)) {
			if(_isfield(val)) {
				SQObjectPtr &o = _values[_member_idx(val)];
				val = (getweakrefs) ? o : _realval(o);
            }
            else {
	...
	bool IsField(const SQObjectPtr &key,SQObjectPtr &val) const
	{
		return _class->_members->Get(key,val) && _isfield(val) != 0;
	}
    void Release() {
        _uiRef++;
