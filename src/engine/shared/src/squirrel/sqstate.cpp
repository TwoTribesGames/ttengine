...
SQSharedState::~SQSharedState()
{
    ...
    SQ_ASSERT(_gc_chain==NULL); //just to proove a theory
    while(_gc_chain){
		_gc_chain->_uiRef--;
        _gc_chain->Release();
    }
...
void RefTable::Resize(SQUnsignedInteger size)
{
    ...
    SQ_ASSERT(nfound == oldnumofslots);
...
void SQStringTable::Remove(SQString *bs)
{
    ...
    SQ_ASSERT(0);//if this fail something is wrong
}
