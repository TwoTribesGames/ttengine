...
dtStatus dtMarkBoxArea(dtTileCacheLayer& layer, const float* orig, const float cs, const float ch,
					   const float* minPos, const float* maxPos, const unsigned char areaId)
{
	const int w = (int)layer.header->width;
	const int h = (int)layer.header->height;
	...
	int minx = (int)floorf((minPos[0]-orig[0])*ics);
	int miny = (int)floorf((minPos[1]-orig[1])*ich);
	int minz = (int)floorf((minPos[2]-orig[2])*ics);
	int maxx = (int)floorf((maxPos[0]-orig[0])*ics);
	int maxy = (int)floorf((maxPos[1]-orig[1])*ich);
	int maxz = (int)floorf((maxPos[2]-orig[2])*ics);
