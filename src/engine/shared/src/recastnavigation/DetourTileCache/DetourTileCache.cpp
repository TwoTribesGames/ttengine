...
dtStatus dtTileCache::addObstacle(const float* minPos, const float* maxPos, dtObstacleRef* result)
{
	if (m_nreqs >= MAX_REQUESTS)
		return DT_FAILURE | DT_BUFFER_TOO_SMALL;
	...
	ob->salt = salt;
	ob->state = DT_OBSTACLE_PROCESSING;
	dtVcopy(ob->pos,    minPos);
	dtVcopy(ob->maxPos, maxPos);
	ob->isBox  = true;
	...
	if (m_nupdate == 0)
	{
		...
		for (int i = 0; i < m_nreqs; ++i)
		{
			...
			if (req->action == REQUEST_ADD)
			{
				...
				// Two Tribes change: fixes obstacle states not being updated if not influencing nav mesh
				if (ob->ntouched == 0)
				{
					// Nothing was influenced by this obstacle: can update state immediately
