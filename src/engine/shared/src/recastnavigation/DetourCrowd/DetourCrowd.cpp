...
dtCrowdAgent* dtCrowd::getAgent_HACK(const int idx)
{
	return &m_agents[idx];
}
...
int dtCrowd::addAgent(const float* pos, const dtCrowdAgentParams* params)
{
	...
	float nearest[3];
	dtVcopy(nearest, pos);	// Two Tribes Addition: make sure nearest[] is initialized to a sensible value
	                        // because findNearestPoly() doesn't necessarily initialize nearestPt, leaving nearest[]
	                        // uninitialized.
	dtPolyRef ref;
...
void dtCrowd::checkPathValidity(dtCrowdAgent** agents, const int nagents, const float dt)
{
	...
	static const float TARGET_REPLAN_DELAY = 1.0f; // seconds
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		dtCrowdAgent* ag = agents[i];
		if (ag->state != DT_CROWDAGENT_STATE_WALKING ||
		    (ag->params.updateFlags & DT_CROWD_UPDATE_POSITION) == 0)
			continue;
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		if (anim->t < ta)
		{
			const float u = tween(anim->t, 0.0f, ta);
