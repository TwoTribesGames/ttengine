...
void Button::OnMouseClickLeft( int /*x*/, int /*y*/, bool bDown )
{
	...
	{
		// Two Tribes change: call onPress *after* onUp
		const bool triggerOnPress = IsHovered() && m_bDepressed;
...
void Button::SetImageAlpha( float f )
{
	...
	m_Image->SetDrawColor( Gwen::Color( 255, 255, 255, static_cast<unsigned char>(255.0f * f) ) );
}
