...
void Menu::ScrollToItem(MenuItem* p_item)
{
	// Two Tribes change
	// Scrolls until the requested MenuItem is visible
	// Used for typing a char in a combobox.
	if (CanScrollV())
	{
		int id = 0;
		for (Base::List::iterator it = m_InnerPanel->Children.begin();
		     it != m_InnerPanel->Children.end(); ++it, ++id)
		{
			...
			if (pChild == p_item)
			{
				...
				float percent = (float)id / (float)(m_InnerPanel->Children.size() - 1);
