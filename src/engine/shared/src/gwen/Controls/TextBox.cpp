...
{
	if ( IsDisabled() ) { return false; }  // Two Tribes change: no text editing when disabled
	if ( c == '\t' ) { return false; }
...
void TextBox::UpdateCaretColor()
{
	...
	m_fNextCaretColorChange = Gwen::Platform::GetTimeInSeconds() + 0.5f;
...
{
	if ( IsDisabled() ) { return; }  // Two Tribes change: no text editing when disabled
...
{
	if ( IsDisabled() ) { return true; }  // Two Tribes change: no text editing when disabled
	if ( !bDown ) { return true; }
...
void TextBox::SetCursorPos( int i )
{
	...
	m_iCursorPos = Clamp(i, 0, TextLength()); // Two Tribes added clamp (was: "= i;")
	m_iCursorLine = 0;
	RefreshCursorBounds();
}
...
void TextBox::SetCursorEnd( int i )
{
	...
	m_iCursorEnd = Clamp(i, 0, TextLength()); // Two Tribes added clamp (was: "= i;")
	RefreshCursorBounds();
}
...
void TextBox::MakeCaratVisible()
{
	...
	else
	{
		...
		if ( m_iAlign & Pos::CenterV ) { y = static_cast<int>( ( Height() - m_Text->Height() ) * 0.5 ); }
...
void TextBoxMultiline::Render( Skin::Base* skin )
{
	...
	if ( m_iCursorPos != m_iCursorEnd )
	{
		...
		//int iFirstChar = 0;   // Two Tribes change: unused variable
		//int iLastChar = 0;    // Two Tribes change: unused variable
		skin->GetRender()->SetDrawColor( Gwen::Color( 50, 170, 255, 200 ) );
		m_rectSelectionBounds.h = m_Text->GetFont()->size + 2;
    
		for(int iLine = iSelectionStartLine; iLine <= iSelectionEndLine; ++iLine)
		{
			//ControlsInternal::Text* line = m_Text->GetLine(iLine);  // Two Tribes change: unused variable
...
void TextBoxMultiline::MakeCaratVisible()
{
	...
	{   
		...
		// If the carat is already in a semi-good position, leave it.
		//int mi = GetPadding().top;  // Two Tribes change: unused variable
		//int ma = Height() - pos.h - GetPadding().bottom;  // Two Tribes change: unused variable
		...
		if ( m_iAlign & Pos::CenterH ) x = static_cast<int>( ( Width() - m_Text->Width() ) * 0.5 );
