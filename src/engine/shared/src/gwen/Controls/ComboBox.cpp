...
GWEN_CONTROL_CONSTRUCTOR( ComboBox )
{
	...
	m_Menu->SetTabbable( false );
	DownArrow* pArrow = new DownArrow( this );
	pArrow->SetComboBox( this );
	m_Button = pArrow;
	m_SelectedItem = NULL;
	m_HoveredItem = NULL; // Two Tribes change
...
bool ComboBox::OnChar( Gwen::UnicodeChar c )
{
	// Two Tribes change
	// Typing a char in a combobox will change the hovered element to the first one that starts with that specific char.
	...
	while ( it != children.end() )
	{
		...
		if ( pChild->GetText().GetUnicode().substr(0, 1) == str)
		{
			setHoveredItem(pChild);
			m_Menu->ScrollToItem(m_HoveredItem);
...
void ComboBox::OpenList()
{
	...
	// Force update to make sure all sizes are correct
	GetCanvas()->DoThink();
	
	if (m_SelectedItem != NULL)
	{
		setHoveredItem(m_SelectedItem);
		m_Menu->ScrollToItem(m_SelectedItem);
	}
...
bool ComboBox::OnKeyReturn( bool /*bDown*/ )
{
	// Two Tribes change
	// Pressing enter will now result in submitting the hovered item
	// Used for typing a char in a combobox.
	if (IsMenuOpen() == false)
	{
		return false;
	}
...
void ComboBox::setHoveredItem(MenuItem* p_item)
{
	// FIXME: Replace this menu hackery with more appropriate ListBox
	// (that already handles hovering and selected states more elegantly)
	m_HoveredItem = p_item;
	
	Base* oldHoveredItem = Gwen::HoveredControl;
	Gwen::HoveredControl = m_HoveredItem;
	
	if (oldHoveredItem != NULL)
	{
		oldHoveredItem->UpdateColours();
	}
	
	m_Menu->OnHoverItem(m_HoveredItem);
}
