...
GWEN_CONTROL_CONSTRUCTOR( ScrollControl )
{
	...
	m_VerticalScrollBar->SetNudgeAmount( 24.25f );
	m_bCanScrollV = true;
...
bool ScrollControl::ContentsAreDocked()
{
	...
	for ( Base::List::iterator iter = m_InnerPanel->Children.begin(); iter != m_InnerPanel->Children.end(); ++iter )
	{
		...
		//if ( pChild->GetDock() == Pos::None ) <-- This breaks combobox scrolling, because items are docked
...
void ScrollControl::SetScrolledAmount(float h, float v)
{
	// Two Tribes change
	// Sets the scrollbar to a certain value (0.0-1.0)
	// Used for typing a char in a combobox.
	UpdateScrollBars();
	
	if (CanScrollH())
	{
		m_HorizontalScrollBar->SetScrolledAmount(h, true);
	}
	
	if (CanScrollV())
	{
		m_VerticalScrollBar->SetScrolledAmount(v, true);
	}
}


float ScrollControl::GetScrolledAmountH() const
{
	// Two Tribes change
	if (m_HorizontalScrollBar != 0)
	{
		return m_HorizontalScrollBar->GetScrolledAmount();
	}
	return 0.0f;
}


void ScrollControl::SetScrolledAmountH(float h)
{
	// Two Tribes change
	// Sets the scrollbar to a certain value (0.0-1.0)
	// Used for typing a char in a combobox.
...
float ScrollControl::GetScrolledAmountV() const
{
	// Two Tribes change
	if (m_VerticalScrollBar != 0)
	{
		return m_VerticalScrollBar->GetScrolledAmount();
	}
	return 0.0f;
}


void ScrollControl::SetScrolledAmountV(float v)
{
	// Two Tribes change
	// Sets the scrollbar to a certain value (0.0-1.0)
	// Used for typing a char in a combobox.
