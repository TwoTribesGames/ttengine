...
Base::~Base()
{
	...
	while ( iter != Children.end() )
	{
		...
		delete pChild;
		iter = Children.begin(); // Two Tribes - Some children will trigger the removal of other Children. So iter is set to begin() again after each delete instead of using the result from erase.
	}
...
void Base::Position( int pos, int xpadding, int ypadding )
{
	...
	if ( pos & Pos::CenterH ) { x = (int)(bounds.x + ( bounds.w - Width() )  * 0.5); }

	if ( pos & Pos::Top ) { y = bounds.y + ypadding; }

	if ( pos & Pos::Bottom ) { y = bounds.y + ( bounds.h - Height() - ypadding ); }

	if ( pos & Pos::CenterV ) { y = (int)(bounds.y + ( bounds.h - Height() )  * 0.5); }
...
void Base::OnMouseLeave()
{
	...
	else if ( GetParent() && GetParent()->GetToolTip() )  // Two Tribes addition: mirror the OnMouseEnter logic
