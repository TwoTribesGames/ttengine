...
int Text::GetClosestCharacter( Gwen::Point p )
{
	...
	for ( Gwen::UnicodeString::size_type i = 0; i < m_String.GetUnicode().length() + 1; i++ )
	{
		Gwen::Rect cp = GetCharacterPosition( static_cast<int>( i ) );
		int iDist = abs( cp.x - p.x ) + abs( cp.y - p.y );   // this isn't proper

		if ( iDist > iDistance ) { continue; }

		iDistance = iDist;
		iChar = static_cast<int>( i );
	}
...
void Text::SplitWords(const Gwen::UnicodeString &s, std::vector<Gwen::UnicodeString> & elems )
{
	...
	for ( int i=0; i<(int)s.length(); i++ ) 
	{
		...
		if ( p.x > w ) 
		{
			//int addSum = GetPadding().left+GetPadding().right;  // Two Tribes changed: why is this variable unused?
			//split words
			str.erase( str.size() - 1 );  // Two Tribes change: original code used pop_back(), but that is a non-standard library extension
			
			// Two Tribes change: do not infinite loop if string will never fit
			if (str.empty()) { break; }
