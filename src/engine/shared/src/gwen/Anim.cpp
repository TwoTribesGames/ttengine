...
// Two Tribes changes: Replaced global variable g_Animations with getGlobalAnimations() function.
static Gwen::Anim::Animation::List& getGlobalAnimations()
{
...
void Gwen::Anim::Add( Gwen::Controls::Base* control, Animation* animation )
{
	...
	getGlobalAnimations()[control].push_back( animation );
}
...
void Gwen::Anim::Cancel( Gwen::Controls::Base* control )
{
	...
	*/
	// HACKFIX: GWEN crash upon exit. By exiting early, we avoid the memory crash upon exit. For now...
	Gwen::Anim::Animation::List& g_Animations = getGlobalAnimations();
	if (g_Animations.empty())
	{
		return;
	}
	// Crashing-upon-application-exit-code below
