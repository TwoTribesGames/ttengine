...
{
	Gwen::String::size_type iOffset = 0;
	Gwen::String::size_type iLength = str.length();
	Gwen::String::size_type iSepLen = seperator.length();
	Gwen::String::size_type i = str.find( seperator, 0 );

	while ( i != Gwen::String::npos )
	{
...
{
	Gwen::UnicodeString::size_type iOffset = 0;
	Gwen::UnicodeString::size_type iLength = str.length();
	Gwen::UnicodeString::size_type iSepLen = seperator.length();
	Gwen::UnicodeString::size_type i = str.find( seperator, 0 );
...
{
	return static_cast<float>( wcstod( str.c_str(), NULL) );
}
...
bool Gwen::Utility::Strings::Wildcard( const TextObject & strWildcard, const TextObject & strHaystack )
{
	...
	UnicodeString::size_type iPos = W.find( L"*", 0 );
