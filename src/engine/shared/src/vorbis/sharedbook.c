...
float *_book_unquantize(const static_codebook *b,int n,int *sparsemap){
  ...
  if(b->maptype==1 || b->maptype==2){
    ...
    float *r=_ogg_calloc((size_t)(n*b->dim),sizeof(*r));
...
int vorbis_book_init_decode(codebook *c,const static_codebook *s){
  ...
  if(n>0){
    ...
    qsort(codep,(size_t)n,sizeof(*codep),sort32a);
    ...
    c->dec_firsttable=_ogg_calloc((size_t)tabn,sizeof(*c->dec_firsttable));
    c->dec_maxlength=0;
	...
	for(j=0;j<(1<<(c->dec_firsttablen-c->dec_codelengths[i]));j++)
	  c->dec_firsttable[orig|(j<<c->dec_codelengths[i])]=(ogg_uint32_t)(i+1);
      }
    }
	...
	if(c->dec_firsttable[bitreverse(word)]==0){
	  ...
	  {
	    unsigned long loval=(unsigned long)lo;
	    unsigned long hival=(unsigned long)(n-hi);
...
long vorbis_book_codeword(codebook *book,int entry){
  if(book->c) /* only use with encode; decode optimizations are
    ...
    return (long)book->codelist[entry];
  return -1;
}
