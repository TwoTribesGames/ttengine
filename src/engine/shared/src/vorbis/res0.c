...
static void res0_pack(vorbis_info_residue *vr,oggpack_buffer *opb){
  ...
  int j,acc=0;
  oggpack_write(opb,(unsigned long)info->begin,24);
  oggpack_write(opb,(unsigned long)info->end,24);

  oggpack_write(opb,(unsigned long)(info->grouping-1),24);  /* residue vectors to group and 
					     code with a partitioned book */
  oggpack_write(opb,(unsigned long)(info->partitions-1),6); /* possible partition choices */
  oggpack_write(opb,(unsigned long)info->groupbook,8);  /* group huffman book */

  /* secondstages is a bitmask; as encoding progresses pass by pass, a
     bitmask of one indicates this partition class has bits to write
     this pass */
  for(j=0;j<info->partitions;j++){
    if(ilog((unsigned int)info->secondstages[j])>3){
      /* yes, this is a minor hack due to not thinking ahead */
      oggpack_write(opb,(unsigned long)info->secondstages[j],3); 
      oggpack_write(opb,1,1);
      oggpack_write(opb,(unsigned long)(info->secondstages[j]>>3),5); 
    }else
      oggpack_write(opb,(unsigned long)info->secondstages[j],4); /* trailing zero */
    acc+=icount((unsigned int)info->secondstages[j]);
  }
  for(j=0;j<acc;j++)
    oggpack_write(opb,(unsigned long)info->booklist[j],8);
...
static vorbis_info_residue *res0_unpack(vorbis_info *vi,oggpack_buffer *opb){
  ...
  for(j=0;j<info->partitions;j++){
    ...
    acc+=icount((unsigned int)cascade);
  }
  for(j=0;j<acc;j++)
...
static vorbis_look_residue *res0_look(vorbis_dsp_state *vd,
  ...
  look->partbooks=_ogg_calloc((size_t)look->parts,sizeof(*look->partbooks));

  for(j=0;j<look->parts;j++){
    int stages=ilog((unsigned int)info->secondstages[j]);
    if(stages){
      if(stages>maxstage)maxstage=stages;
      look->partbooks[j]=_ogg_calloc((size_t)stages,sizeof(*look->partbooks[j]));
      for(k=0;k<stages;k++)
...
static long **_01class(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  long **partword=_vorbis_block_alloc(vb,(long)(ch*sizeof(*partword)));
  ...
  for(i=0;i<ch;i++){
    partword[i]=_vorbis_block_alloc(vb,(long)(n/samples_per_partition*sizeof(*partword[i])));
...
static long **_2class(vorbis_block *vb,vorbis_look_residue *vl,float **in,
  ...
  partword[0]=_vorbis_block_alloc(vb,(long)(n*ch/samples_per_partition*sizeof(*partword[0])));
...
static int _01inverse(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  if(n>0){
    ...
    for(j=0;j<ch;j++)
      partword[j]=_vorbis_block_alloc(vb,(long)(partwords*sizeof(*partword[j])));
...
static int res2_forward(oggpack_buffer *opb,
  ...
  float *work=_vorbis_block_alloc(vb,(long)(ch*n*sizeof(*work)));
  for(i=0;i<ch;i++){
    float *pcm=in[i];
    if(nonzero[i])used++;
...
static int res2_inverse(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  if(n>0){
    int partvals=n/samples_per_partition;
    int partwords=(partvals+partitions_per_word-1)/partitions_per_word;
    int **partword=_vorbis_block_alloc(vb,(long)(partwords*sizeof(*partword)));
