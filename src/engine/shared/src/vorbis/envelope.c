...
void _ve_envelope_init(envelope_lookup *e,vorbis_info *vi){
  ...
  e->mdct_win=_ogg_calloc((size_t)n,sizeof(*e->mdct_win));
  mdct_init(&e->mdct,n);
  ...
  for(j=0;j<VE_BANDS;j++){
    n=e->band[j].end;
    e->band[j].window=_ogg_malloc((size_t)(n*sizeof(*e->band[0].window)));
    for(i=0;i<n;i++){
  ...
  e->filter=_ogg_calloc((size_t)(VE_BANDS*ch),sizeof(*e->filter));
  e->mark=_ogg_calloc((size_t)e->storage,sizeof(*e->mark));
