...
int vorbis_staticbook_pack(const static_codebook *c,oggpack_buffer *opb){
  ...
  oggpack_write(opb,(unsigned long)0x564342,24);
  oggpack_write(opb,(unsigned long)c->dim,16);
  oggpack_write(opb,(unsigned long)c->entries,24);
  ...
  if(ordered){
    ...
    oggpack_write(opb,(unsigned long)(c->lengthlist[0]-1),5); /* 1 to 32 */
    ...
    for(i=1;i<c->entries;i++){
      ...
      if(this>last){
	for(j=last;j<this;j++){
	  oggpack_write(opb,(unsigned long)(i-count),_ilog((unsigned int)(c->entries-count)));
	  count=i;
	}
      }
    }
    ...
    if(i==c->entries){
      ...
      for(i=0;i<c->entries;i++)
	oggpack_write(opb,(unsigned long)(c->lengthlist[i]-1),5);
    }else{
  ...
  oggpack_write(opb,(unsigned long)c->maptype,4);
  switch(c->maptype){
  case 0:
    /* no mapping */
    break;
  case 1:case 2:
    ...
    oggpack_write(opb,(unsigned long)c->q_min,32);
    oggpack_write(opb,(unsigned long)c->q_delta,32);
    oggpack_write(opb,(unsigned long)(c->q_quant-1),4);
    oggpack_write(opb,(unsigned long)c->q_sequencep,1);
	...
	oggpack_write(opb,(unsigned long)labs(c->quantlist[i]),c->q_quant);
...
STIN long decode_packed_entry_number(codebook *book, oggpack_buffer *b){
  ...
  if (lok >= 0) {
    long entry = (long)book->dec_firsttable[lok];
