...
static float ***setup_tone_curves(float curveatt_dB[P_BANDS],float binHz,int n,
  ...
  for(i=0;i<P_BANDS;i++){
	...
	if(adj<0. && center_boost>0)adj=0.0f;
	if(adj>0. && center_boost<0)adj=0.0f;
	workc[i][j][k]+=adj;
      }
    }
...
void _vp_noisemask(vorbis_look_psy *p,
  ...
  bark_noise_hybridmp(n,p->bark,work,logmask,0.0f,
...
static float dipole_hypot(float a, float b){
  if(a>0.){
    if(b>0.f)return (float)sqrt(a*a+b*b);
...
float **_vp_quantize_couple_memo(vorbis_block *vb,
  ...
  int i,j,n=p->n;
  float **ret=_vorbis_block_alloc(vb,(long)(vi->coupling_steps*sizeof(*ret)));
  int limit=g->coupling_pointlimit[p->vi->blockflag][PACKETBLOBS/2];
  
  for(i=0;i<vi->coupling_steps;i++){
    float *mdctM=mdct[vi->coupling_mag[i]];
    float *mdctA=mdct[vi->coupling_ang[i]];
    ret[i]=_vorbis_block_alloc(vb,(long)(n*sizeof(**ret)));
    for(j=0;j<limit;j++)
...
int **_vp_quantize_couple_sort(vorbis_block *vb,
  ...
  if(p->vi->normal_point_p){
    int i,j,k,n=p->n;
    int **ret=_vorbis_block_alloc(vb,(long)(vi->coupling_steps*sizeof(*ret)));
	...
	qsort(work,(size_t)partition,sizeof(*work),apsort);
...
void hf_reduction(vorbis_info_psy_global *g,
  ...
  int i,j,n=p->n, de=(int)(0.3f*p->m_val);
  int limit=g->coupling_pointlimit[p->vi->blockflag][PACKETBLOBS/2];
  ...
  for(i=0; i<vi->coupling_steps; i++){
    ...
    for(j=limit; j<n; j++) 
      mdct[i][j] *= (1.0f - de*((float)(j-limit) / (float)(n-limit)));
  }
}
