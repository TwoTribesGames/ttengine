...
static uint8_t *_frm1 = NULL;
static uint8_t *_frm2 = NULL;
static uint8_t *_frm3 = NULL;

extern uint32_t qRGB_COLOR_MASK[2];

static void Init()
{
  _frm1 = (uint8_t *)calloc(322*242,4);
  // 1 frame ago
  _frm2 = (uint8_t *)calloc(322*242,4);
  // 2 frames ago
  _frm3 = (uint8_t *)calloc(322*242,4);
  // 3 frames ago
}
...
void InterframeCleanup()
{
  ...
  if(_frm1)
    free(_frm1);
  if(_frm2 && (_frm1 != _frm2))
    free(_frm2);
  if(_frm3 && (_frm1 != _frm3) && (_frm2 != _frm3))
    free(_frm3);
  _frm1 = _frm2 = _frm3 = NULL;
}
...
static void SmartIB_MMX(uint8_t *srcPtr, uint32_t srcPitch, int width, int starty, int height)
{
  ...
  uint16_t *src1 = (uint16_t *)_frm1 + srcPitch * starty / 2;
  uint16_t *src2 = (uint16_t *)_frm2 + srcPitch * starty / 2;
  uint16_t *src3 = (uint16_t *)_frm3 + srcPitch * starty / 2;
...
static void SmartIB32_MMX(uint8_t *srcPtr, uint32_t srcPitch, int width, int starty, int height)
{
  ...
  uint32_t *src1 = (uint32_t *)_frm1 + starty * srcPitch / 4;
  uint32_t *src2 = (uint32_t *)_frm2 + starty * srcPitch / 4;
  uint32_t *src3 = (uint32_t *)_frm3 + starty * srcPitch / 4;
...
{
  uint16_t *src0 = (uint16_t *)srcPtr + starty * srcPitch / 2;
  uint16_t *src1 = (uint16_t *)_frm1 + starty * srcPitch / 2;
