...
bool CPUReadBatteryFile(const char* fileName)
{
    ...
    fseek(file, 0, SEEK_SET);
    systemSaveUpdateCounter = SYSTEM_SAVE_NOT_UPDATED;
	int detectedSaveType = GBA_SAVE_NONE;
    ...
    if (size == 512 || size == 0x2000) {
		...
		detectedSaveType = GBA_SAVE_EEPROM;
    } else {
        if (size == 0x20000) {
			...
			detectedSaveType = GBA_SAVE_FLASH;
	...
	// MR: Without this hackery, the saveType isn't updated and stays on 5 (GBA_SAVE_NONE)
	if (detectedSaveType != GBA_SAVE_NONE)
	{
