...
static const tt::fs::FilePtr blargg_fopen( const char path [] )
{
	tt::fs::FilePtr file;
	if ( path )
	{
		file = tt::fs::open(path, tt::fs::OpenMode_Read );
	}
...
static blargg_err_t blargg_fopen( tt::fs::FilePtr& p_fileOUT, const char path [] )
{
	errno = 0;
	p_fileOUT = blargg_fopen( path );
	if (p_fileOUT == nullptr)
	{
		#ifdef ENOENT
			if ( errno == ENOENT )
...
static blargg_err_t blargg_fsize( tt::fs::FilePtr& f, long* out )
{
	if (f == nullptr)
	{
...
blargg_err_t Std_File_Reader::seek_v( int n )
{
	if (file_ != nullptr)
	{
		if (tt::fs::seek(file_, n, tt::fs::SeekPos_Set))
		{
			return blargg_ok;
		}
	}
