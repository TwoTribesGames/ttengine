...
tt::engine::anim2d::TweenType Tween::tweenTypeFromString(const std::string& p_name)
{
...
std::string Tween::tweenTypeToString(const TweenType& p_tweentype)
{
	switch(p_tweentype)
	{
...
const tt::engine::anim2d::tween::Easing& Tween::tweenFactory(TweenType p_tweentype)
{
	switch(p_tweentype)
	{
	...
	default: TT_PANIC("Unknown timetype"); return tween::g_Quad;
	}
}
...
{
	return -c * tt::math::cos(t / d * (tt::math::pi / 2)) + c + b;
}
real Sine::easeOut(real t, real b , real c, real d) const
{
	return c * tt::math::sin(t / d * (tt::math::pi / 2)) + b;
}

real Sine::easeInOut(real t, real b , real c, real d) const
{
	return -c / 2 * (tt::math::cos(tt::math::pi * t / d) - 1) + b;
}
...
{
	return (t == 0) ? b : c * tt::math::pow(static_cast<real>(2), 10.0f * (t / d - 1.0f)) + b;
}
real Expo::easeOut(real t,real b , real c, real d) const
{
	return (t == d) ? b + c : c * ( -tt::math::pow(static_cast<real>(2), -10 * t / d) + 1) + b;
}
...
real Expo::easeInOut(real t,real b , real c, real d) const
{
	...
	if (t == d) return b + c;
	t /= d / 2;
	if (t < 1) return c / 2 * tt::math::pow(static_cast<real>(2), 10 * (t - 1)) + b;
	--t;
	return c / 2 * (-tt::math::pow(static_cast<real>(2), -10 * t) + 2) + b;
}
...
real Elastic::easeIn (real t,real b , real c, real d) const
{
	...
	real s = p / 4;
	t -= 1;
	real postFix = a * tt::math::pow(static_cast<real>(2), 10 * t); // this is a fix, again, with post-increment operators
	return -(postFix * tt::math::sin((t * d - s) * (2 * tt::math::pi) / p)) + b;
}
...
real Elastic::easeOut(real t,real b , real c, real d) const
{
	...
	real a = c;
	real s = p / 4;
	return a * tt::math::pow(static_cast<real>(2), -10 * t) * 
		tt::math::sin((t * d - s) * (2 * tt::math::pi) / p) + c + b;
}
...
real Elastic::easeInOut(real t,real b , real c, real d) const
{
	...
	if (t < 1) 
	{
		real postFix = a * tt::math::pow(static_cast<real>(2), 10 * t); // postIncrement is evil
		return -.5f * (postFix * tt::math::sin((t * d - s) * (2 * tt::math::pi) / p)) + b;
	}
	real postFix = a * tt::math::pow(static_cast<real>(2), -10 * t); // postIncrement is evil
	return postFix * tt::math::sin((t * d - s) * (2 * tt::math::pi) / p) * .5f + c + b;
}
...
{  
	t /= d;
	return -c * (tt::math::sqrt(1 - t * t) - 1) + b;
}
real Circ::easeOut(real t,real b , real c, real d) const
{
	t = t / d - 1;
	return c * tt::math::sqrt(1 - t * t) + b;
}

real Circ::easeInOut(real t,real b , real c, real d) const
{
	t /= d / 2;
	if (t < 1) return -c / 2 * (tt::math::sqrt(1 - t * t) - 1) + b;
	t -= 2;
	return c / 2 * (tt::math::sqrt(1 - t * t) + 1) + b;
}
