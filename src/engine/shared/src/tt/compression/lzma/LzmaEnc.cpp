...
#define BSR2_RET(pos, res) { UInt32 tempVar = 6 + ((kNumLogBits - 1) & \
  (0 - (((((UInt32)1 << (kNumLogBits + 6)) - 1) - pos) >> 31))); \
  res = p->g_FastPos[pos >> tempVar] + (tempVar * 2); }
/*
        ...
        if (/*_maxMode && */lenTest == matches[offs])
        {
          ...
          if (lenTest2 >= 2)
          {
            ...
            /* for (; lenTest2 >= 2; lenTest2--) */
            {
              ...
              {
                opt2->price = curAndLenPrice2;
                opt2->posPrev = cur + lenTest + 1;
                opt2->backPrev = 0;
                opt2->prev1IsChar = True;
                opt2->prev2 = True;
                opt2->posPrev2 = cur;
                opt2->backPrev2 = curBack + LZMA_NUM_REPS;
              }
            }
          }
          offs += 2;
