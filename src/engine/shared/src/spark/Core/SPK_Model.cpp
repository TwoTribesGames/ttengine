...
namespace SPK
{
	...
	Model::Model(int p_enableFlag,int p_mutableFlag,int p_randomFlag,int p_interpolatedFlag) :
		Registerable(),
		paramsSize(0),
		nbEnableParams(0),
		nbMutableParams(0),
		...
		immortal(false)
	{
		p_enableFlag |= FLAG_RED | FLAG_GREEN | FLAG_BLUE; // Adds the color parameters to the enable flag
		this->enableFlag = p_enableFlag & ((1 << (NB_PARAMS + 1)) - 1); // masks the enable flag with the existing parameters
		this->interpolatedFlag = p_interpolatedFlag & this->enableFlag; // masks the interpolated flag with the enable flag
		this->mutableFlag = p_mutableFlag & this->enableFlag; // masks the mutable flag with the enable flag
		this->mutableFlag &= ~this->interpolatedFlag; // a param cannot be both interpolated and mutable
		this->randomFlag = p_randomFlag & this->enableFlag; // masks the random flag with the enable flag
		...
		for (size_t i = 0; i < NB_PARAMS; ++i)
		{
			...
			indices[i] = static_cast<int>(paramsSize);
		...
		if (nbEnableParams > 0)
		{
			...
			for (size_t i = 0; i < NB_PARAMS; ++i)
				if (isEnabled(static_cast<ModelParam>(i)))
					enableParams[index++] = static_cast<int>(i);
		}
		else
			enableParams = NULL;
		...
		if (nbMutableParams > 0)
		{
			...
			for (size_t i = 0; i < NB_PARAMS; ++i)
				if (isMutable(static_cast<ModelParam>(i)))
					mutableParams[index++] = static_cast<int>(i);
		}
		else
			mutableParams = NULL;
		...
		if (nbInterpolatedParams > 0)
		{
			...
			for (size_t i = 0; i < NB_PARAMS; ++i)
				if (isInterpolated(static_cast<ModelParam>(i)))
					interpolatedParams[index++] = static_cast<int>(i);
		}
		else
