...
#include <spark/Core/SPK_Factory.h>
...
namespace SPK
{
	...
	Registerable* Group::findByName(const std::string& p_name)
	{
		Registerable* object = Registerable::findByName(p_name);
		if (object != NULL)
			return object;

		object = model->findByName(p_name);
		if (object != NULL)
			return object;

		if (renderer != NULL)
		{
			object = renderer->findByName(p_name);
			if (object != NULL)
				return object;
		}
	...
	void Group::setRenderer(Renderer* p_renderer)
	{
		decrementChildReference(this->renderer);
		incrementChildReference(p_renderer);

		if ((bufferManagement)&&(p_renderer != this->renderer))
		{
			if (this->renderer != NULL) this->renderer->destroyBuffers(*this);
			if (p_renderer != NULL) p_renderer->createBuffers(*this);
		}
	...
	bool Group::update(float deltaTime)
	{
		...
		if ((sortingEnabled)&&(pool.getNbActive() > 1))
			sortParticles(0,static_cast<int>(pool.getNbActive() - 1));
	...
	Buffer* Group::createBuffer(const std::string& p_ID,const BufferCreator& creator,unsigned int flag,bool swapEnabled) const
	{
		destroyBuffer(p_ID);
		...
		additionalBuffers.insert(std::pair<std::string,Buffer*>(p_ID,buffer));
		if (swapEnabled)
	...
	void Group::destroyBuffer(const std::string& p_ID) const
	{
		std::map<std::string,Buffer*>::iterator it = additionalBuffers.find(p_ID);
	...
	Buffer* Group::getBuffer(const std::string& p_ID,unsigned int flag) const
	{
		Buffer* buffer = getBuffer(p_ID);
	...
	Buffer* Group::getBuffer(const std::string& p_ID) const
	{
		std::map<std::string,Buffer*>::const_iterator it = additionalBuffers.find(p_ID);
