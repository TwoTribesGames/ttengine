...
namespace SPK
{
	...
	void System::setCameraPosition(const Vector3D& p_cameraPosition)
	{
		System::cameraPosition = p_cameraPosition;
	}
	...
	void System::useConstantStep(float p_constantStep)
	{
		stepMode = STEP_CONSTANT;
		System::constantStep = p_constantStep;
	}

	void System::useAdaptiveStep(float p_minStep,float p_maxStep)
	{
		stepMode = STEP_ADAPTIVE;
		System::minStep = p_minStep;
		System::maxStep = p_maxStep;
	}
	...
	Registerable* System::findByName(const std::string& p_name)
	{
