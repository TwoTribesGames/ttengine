...
namespace SPK
{
	Particle::Particle(Group* p_group,size_t p_index) :
		group(p_group),
		index(p_index),
		data(p_group->particleData + p_index),
		currentParams(p_group->particleCurrentParams + p_index * p_group->model->getSizeOfParticleCurrentArray()),
		extendedParams(p_group->particleExtendedParams + p_index * p_group->model->getSizeOfParticleExtendedArray())
	{
		init();
	}
	...
	void Particle::interpolateParameters()
	{
		...
		{
			size_t idx = model->interpolatedParams[i];
			size_t enableIndex = model->particleEnableIndices[idx];
			currentParams[enableIndex] = model->interpolators[idx]->interpolate(*this,static_cast<ModelParam>(idx),interpolatedIt[0],interpolatedIt[1],interpolatedIt[2]);
			interpolatedIt += 3;
		}
	}
	...
	bool Particle::update(float deltaTime)
	{
		...
		if (!model->immortal)
		{
			...
			{
				size_t idx = model->mutableParams[i];
				size_t enableIndex = model->particleEnableIndices[idx];
