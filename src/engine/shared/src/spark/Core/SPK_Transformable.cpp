...
namespace SPK
{
	...
	void Transformable::updateTransform(const Transformable* p_parent)
	{
		if (isUpdateNotified() ||											// the local transform or instance param have been updated
			p_parent != this->parent ||										// the p_parent has changed
			(p_parent != NULL && lastParentUpdate != p_parent->currentUpdate))	// the p_parent transform has been modified
		{
			if (p_parent == NULL)
				memcpy(world,local,sizeof(float) * TRANSFORM_LENGTH);
			else if (isLocalIdentity())
			{
				memcpy(world,p_parent->world,sizeof(float) * TRANSFORM_LENGTH);
				lastParentUpdate = p_parent->lastUpdate;
			}
			else
			{
				multiply(world,p_parent->world,local);
				lastParentUpdate = p_parent->lastUpdate;
			}
