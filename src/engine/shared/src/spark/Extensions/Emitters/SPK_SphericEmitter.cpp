...
namespace SPK
{
	...
	SphericEmitter::SphericEmitter(const Vector3D& p_direction,float p_angleA,float p_angleB) :
		Emitter()
	{
		setDirection(p_direction);
		setAngles(p_angleA,p_angleB);
	}

	void SphericEmitter::setDirection(const Vector3D& p_direction)
	{
		tDirection = p_direction;
		computeMatrix();
