...
namespace SPK
{
	AABox::AABox(const Vector3D& p_position,const Vector3D& p_dimension) :
		Zone(p_position)
	{
		setDimension(p_dimension);
	}

	void AABox::setDimension(const Vector3D& p_dimension)
	{
		dimension.set(std::max(0.0f,p_dimension.x),std::max(0.0f,p_dimension.y),std::max(0.0f,p_dimension.z));
	}
	...
	{
		// Get a second point based on the direction of box position -> point which is outside of the box.
		const Vector3D boxPos(getTransformedPosition());
		Vector3D diff(point - boxPos);
		diff *= dimension.getNorm(); // Scale direction so it's outside the box for sure.
		
		Vector3D intersectDummy; // Needed so normal is returned
		Vector3D normal;
		
		// Get intersect from outside point to center of the box.
		if (intersects(boxPos + diff, boxPos, &intersectDummy, &normal) == false)
		{
