...
#include <spark/Extensions/Modifiers/SPK_ModifierGroup.h>

namespace SPK
{
	ModifierGroup::ModifierGroup(Zone* p_zone,ModifierTrigger p_trigger) :
		Modifier(INSIDE_ZONE | OUTSIDE_ZONE | INTERSECT_ZONE | ENTER_ZONE | EXIT_ZONE,p_trigger,false,false,p_zone),
		modifiers(),
		globalZone(false),
	...
	Registerable* ModifierGroup::findByName(const std::string& p_name)
	{
		Registerable* object = Modifier::findByName(p_name);
		if (object != NULL)
			return object;
