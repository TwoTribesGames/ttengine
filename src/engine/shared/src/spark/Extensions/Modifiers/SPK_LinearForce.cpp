...
#include <spark/Extensions/Modifiers/SPK_LinearForce.h>
#include <spark/Core/SPK_Particle.h>

namespace SPK
{
	LinearForce::LinearForce(Zone* p_zone,ModifierTrigger /*trigger*/,const Vector3D& p_force,ForceFactor type,ModelParam param) :
		Modifier(ALWAYS | INSIDE_ZONE | OUTSIDE_ZONE,ALWAYS,false,false,p_zone),
		force(p_force),
		tForce(p_force),
		factorType(type),
		factorParam(param)
