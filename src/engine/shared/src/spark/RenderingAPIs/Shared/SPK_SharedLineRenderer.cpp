...
#include <spark/Core/SPK_ArrayBuffer.h>
...
namespace Shared
{
	...
	DX9LineRenderer::~DX9LineRenderer()
	{
	...
	void DX9LineRenderer::createBuffers(const Group& group)
	{
		...
		indexBuffer->Lock(0, group.getParticles().getNbReserved()*6*sizeof(short),
			(void**)&indexIterator, D3DLOCK_DISCARD);
...
//#define _DX9LINERENDERER_CLOCKWISE_
	...
	void DX9LineRenderer::render(const Group& group)
	{
		...
		if( group.getNbParticles() != 0 )
		{
			...
			if( DX9VertexBuffer->Lock(0, 0, &ptr, D3DLOCK_DISCARD) == D3D_OK )
			{
	...
	void DX9LineRenderer::render(const Group& group)
	{
		...
		if( group.getNbParticles() != 0 )
		{
			hr = gpuBuffer->Lock(0, group.getNbParticles()*4*sizeof(LineVertex), (void**)&gpuIterator, D3DLOCK_DISCARD);
	...
	bool DX9LineRenderer::DX9CreateBuffers(const Group& group)
	{
		...
		if( DX9Info::getDevice()->CreateVertexBuffer(
			group.getParticles().getNbReserved() * 2 * sizeof(LineVertex),
