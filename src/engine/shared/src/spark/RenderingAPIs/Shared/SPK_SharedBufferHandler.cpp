...
namespace Shared
{
	bool TTBufferHandler::DX9DestroyAllBuffers()
	{
	...
	bool TTBufferHandler::DX9PrepareBuffers(const Group& group)
	{
...
//					std::cout << "DX9CreateBuffers echec" << std::endl;
	...
	bool TTBufferHandler::DX9Bind(const Group& group, int key, void** to)
	{
	...
	bool TTBufferHandler::DX9Release(const Group& group, int key)
	{
		DX9BuffersKey = std::pair<const Group *, int>(&group, key);
		DX9BuffersIt = DX9Buffers.find(DX9BuffersKey);
