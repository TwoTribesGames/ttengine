...
namespace Shared
{
	...
	bool DX9LineTrailRenderer::DX9CreateBuffers(const Group& group)
	{
		...
		if( DX9Info::getDevice()->CreateVertexBuffer(
			group.getParticles().getNbReserved() * (nbSamples + 2) * sizeof(D3DXVECTOR3),
		...
		if( DX9Info::getDevice()->CreateVertexBuffer(
			group.getParticles().getNbReserved() * (nbSamples + 2) * sizeof(D3DCOLOR),
			D3DUSAGE_WRITEONLY|D3DUSAGE_DYNAMIC,
			D3DFVF_DIFFUSE,
			D3DPOOL_DEFAULT, &vb, NULL) != S_OK )
		{
	...
	void DX9LineTrailRenderer::DX9ReleaseVertexDeclarations()
	{
		SAFE_RELEASE( pVertexDecl );
	}
}}
