...
int ogg_stream_packetin(ogg_stream_state *os,ogg_packet *op){
  ...
  memcpy(os->body_data+os->body_fill,op->packet,(ogg_uint32_t)(op->bytes));
...
char *ogg_sync_buffer(ogg_sync_state *oy, long size){
  ...
  if(oy->returned){
    oy->fill-=oy->returned;
    if(oy->fill>0)
      memmove(oy->data,oy->data+oy->returned,(ogg_uint32_t)(oy->fill));
    oy->returned=0;
  }
  ...
  if(size>oy->storage-oy->fill){
    ...
    if(oy->data)
      oy->data=_ogg_realloc(oy->data,(ogg_uint32_t)(newsize));
    else
      oy->data=_ogg_malloc((ogg_uint32_t)newsize);
    oy->storage=newsize;
  }
...
long ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
  ...
  {
    unsigned char *page2=oy->data+oy->returned;
    long bytes2;
    ...
    oy->unsynced=0;
    oy->returned+=(bytes2=oy->headerbytes+oy->bodybytes);
    oy->headerbytes=0;
    oy->bodybytes=0;
    return(bytes2);
  }
 ...
 sync_fail:
  ...
  next=memchr(page+1,'O',(ogg_uint32_t)(bytes-1));
  if(!next)
...
int ogg_stream_pagein(ogg_stream_state *os, ogg_page *og){
  ...
  /* clean up 'returned data' */
  {
    ...
    if(br){
      os->body_fill-=br;
      if(os->body_fill)
	memmove(os->body_data,os->body_data+br,(ogg_uint32_t)(os->body_fill));
      os->body_returned=0;
    }
  ...
  if(bodysize){
    ...
    memcpy(os->body_data+os->body_fill,body,(ogg_uint32_t)(bodysize));
