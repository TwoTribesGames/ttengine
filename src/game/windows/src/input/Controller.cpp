...
#if 0 // use this if to toggle between SDL (0) or xbox controller (1) input.
#include               <tt/input/Xbox360Controller.h>
#define GAMEPAD_CONTROLLER_CLASS Xbox360Controller
#else
#include               <tt/input/SDLJoypadController.h>
#define GAMEPAD_CONTROLLER_CLASS SDLJoypadController
#endif
...
void Controller::update(real p_time)
{
	...
	for(u32 i = 0; i < 4; ++i)
	{
		...
		if(tt::input::GAMEPAD_CONTROLLER_CLASS::isConnected(index))
		{
	...
	if (force > 0.0f && m_rumbleTime > 0.0f)
	{
		...
		tt::input::GAMEPAD_CONTROLLER_CLASS::setRumble(activeController, force);
	}
...
void Controller::updatePlatformState()
{
	...
	using tt::input::GAMEPAD_CONTROLLER_CLASS;
	...
	const GAMEPAD_CONTROLLER_CLASS&  gamepad(GAMEPAD_CONTROLLER_CLASS::getState(activeController));
	...
	// Assign sticks
	{
		...
		newInput.faceUp.update(gamepad.y.down || kbd.keys[tt::input::Key_2].down); 
		newInput.faceLeft.update(gamepad.x.down || kbd.keys[tt::input::Key_1].down);
		
		newInput.direction = getNormalizedStick(gamepad.lstick);
		newInput.scroll    = getNormalizedStick(gamepad.rstick);
		
		rawLeft  = isKeyDown(BindableAction_Left)  || gamepad.left.down;
		rawRight = isKeyDown(BindableAction_Right) || gamepad.right.down;
		rawUp    = isKeyDown(BindableAction_Up)    || gamepad.up.down;
		rawDown  = isKeyDown(BindableAction_Down)  || gamepad.down.down;
	...
	// Weapon selection
	bool isDownSelectWeapon1 = false;
	bool isDownSelectWeapon2 = false;
	bool isDownSelectWeapon3 = false;
	bool isDownSelectWeapon4 = false;
	...
	case GamepadControlScheme_A2:
	case GamepadControlScheme_B2:
		isDownSelectWeapon1 = gamepad.x.down || isKeyDown(BindableAction_SelectWeapon1);
		isDownSelectWeapon2 = gamepad.a.down || isKeyDown(BindableAction_SelectWeapon2);
		isDownSelectWeapon3 = gamepad.y.down || isKeyDown(BindableAction_SelectWeapon3);
		isDownSelectWeapon4 = gamepad.b.down || isKeyDown(BindableAction_SelectWeapon4);
		break;
	...
	newInput.selectWeapon1.update(isDownSelectWeapon1);
	newInput.selectWeapon2.update(isDownSelectWeapon2);
	newInput.selectWeapon3.update(isDownSelectWeapon3);
	newInput.selectWeapon4.update(isDownSelectWeapon4);
...
void Controller::stopRumbleImpl(bool p_immediately)
{
	...
	if (p_immediately)
	{
		tt::input::GAMEPAD_CONTROLLER_CLASS::setRumble(activeController, 0.0f);
	}
}
